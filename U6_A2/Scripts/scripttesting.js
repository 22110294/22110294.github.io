...
logoButton.addEventListener("click", function() {
    ...
    let correctPassword = "letmein123"; // Change this to your actual password

    if (password === correctPassword) {
        window.location.href = "abouttesting.html"; // Change to your actual page path
    } else {
...
const slideHeight = 720; // Height of each image
const marginTop = 16;

// Sample person information (replace with actual data)
const persons = [
...
const scrollThumb = () => {
    ...
    updatePersonInfo(index); // Update person info when scrolling
};
...
const scrollToElement = el => {
    ...
    updatePersonInfo(index); // Update info when clicking on a thumbnail
};

// Update the person info in the section
...
// Add event listener for each thumbnail image
document.querySelectorAll('.thumbnails img').forEach(el => {
    el.addEventListener('click', () => scrollToElement(el));
});

// Add scroll event listener to update the scrollbar thumb
